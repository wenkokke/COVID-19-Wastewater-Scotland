...
RAINFALL_DATA_COLUMNS = ['station_no', 'timestamp', 'value']
if not RAINFALL_DATA_FILE.exists():
    with open(
            RAINFALL_DATA_FILE,
            'w',
            newline='',
            encoding=RAINFALL_DATA_ENCODING
        ) as rainfall_data_fp:
        ...
        for station_data in STATIONS_DATA:
            ...
            stdout.write("\r")
            stdout.write("[{:<3}] {:<40}".format(
                station_data_response.status_code,
                station_name
            ))
            station_data_samples = station_data_response.json()
            stdout.write("\r")
            stdout.write("[{:<3}] {:<40} ({} samples)".format(
                station_data_response.status_code,
                station_name,
                len(station_data_samples)
            ))
            stdout.write("\n")
